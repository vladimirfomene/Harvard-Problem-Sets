...
void draw(void)
{
    //space character for spacing
    char space = ' ';
    //cycle through the grid each time and printing the grid characters
    for(int row = 0; row < DIMENSION; row++)
        for(int col = 0; col < DIMENSION; col++)
        {
            printf("%c",grid[row][col]);
            printf("%c",space);
            //newline for the end of each row
            if(col == DIMENSION - 1)
                printf("\n");
        }
...
bool lookup(string s)
{
    ...
    {
        if(strcmp(dictionary.words[i].letters, s) == 0)
        {
            if(dictionary.words[i].found == false)
            {
    ...
    while(end >= begin)
    {
        ...
        if(strcmp(dictionary.words[midpoint].letters, s) < 0)
            begin = midpoint + 1;
        else if(strcmp(dictionary.words[midpoint].letters, s) > 0)
            end = midpoint - 1;
        else
        {
             //check whether s was already found
            //if no flag it and return true
            if(dictionary.words[midpoint].found == false)
            {
...
{
    int y = 1;
    //cycle through the grid rotating each letter by 90 degrees
    for(int row = 0; row < DIMENSION; row++)
    {
        for(int col = 0; col < DIMENSION; col++)
        {
            grid[col][DIMENSION - y] = grid[row][col]; 
